...
def create_web_app(download_dir: str, bot_token: str) -> Flask:
    ...
    def list_files():
        ...
        try:
            ...
            return jsonify({
                'current_path': str(target_dir.relative_to(app.download_dir)) if path_param else '',
                'items': items,
                'total_size': format_size(sum(
    ...
    def search_files():
        ...
        try:
            for file_path in app.download_dir.rglob('*'):
                ...
                try:
                    # Match by name
                    if query and query in file_path.name.lower():
                        results.append(get_file_info(file_path))
                    # Match by extension
                    elif file_type and file_path.suffix.lower() == f".{file_type}":
                        results.append(get_file_info(file_path))
                except (OSError, PermissionError):
                    continue
